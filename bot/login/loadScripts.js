...
module.exports = async function (api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, createLine) {
	...
	for (const folderModules of folders) {
		const makeColor = folderModules == "cmds" ?
			createLine("ULLASH LOAD COMMANDS") :
			createLine("ULLASH LOAD COMMANDS EVENT");
		...
		for (const file of Files) {
			...
			loading.info('ULLASH LOADED', `${colors.green(`${commandLoadSuccess}`)}${commandError.length ? `, ${colors.red(`${commandError.length}`)}` : ''}`);
		}
		console.log("\r");
		if (commandError.length > 0) {
			log.err("ULLASH LOADED", getText('loadScripts', 'loadScriptsError', colors.yellow(text)));
